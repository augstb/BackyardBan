 ...
 * Replaces net.md_5.bungee.config.Configuration so config/locale/ban-data loading isn't tied to
 * bungeecord-config's YamlConfiguration, which always builds its Yaml with a plain, unsafe
 * SnakeYAML Constructor (see YamlStore).
 */
public final class YamlDocument {
    ...
    // Top-level keys only (banlist/baniplist/knownplayers store one entry per player/IP at the top level).
    public Set<String> getKeys() { return new LinkedHashSet<>(data.keySet()); }
